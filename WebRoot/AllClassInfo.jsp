		...
		$.ajax({
	        ...
	        // 回调函数，接受服务器端返回给客户端的值，即result值
	         success:function(list2){  
	        	 $("#major").html("");
	        	 $("#major").append('<option select="selected">'+'-请选择-'+'</option>');
	...
	<div id="content">
		<div id="content-header">
			...
			<div style="margin-left:20px;margin-top:18px;color:blue">提示:学院和专业必须同时查询，为了提高选课效率，请同学们使用查询过滤功能。</div>
				<form action="ClassInfo_findByCondition.action" method="post" onsubmit="return check()">
					<table style="margin-left:20px;margin-top:18px">
						<tr>
							<td style="font-size:15px;color:red">开设学院：</td>
							<td><select
								style="width:135px;margin-top:10px;border-radius:15px"
							...
							<td style="font-size:15px">入校年份:</td>
							<td><input type="text" class="inputStyle"
								style="width:124px;margin-top:10px;border-radius:15px" name="grade"></td>
							<td style="font-size:15px">学习方向：</td>
							<td><input type="text" class="inputStyle"
								style="width:124px;margin-top:10px;border-radius:15px" name="studyDirection"></td>
							<td style="font-size:15px">辅导员：</td>
							<td><input type="text" class="inputStyle"
								style="width:124px;margin-top:10px;border-radius:15px" name="teacher">
							</td>
						</tr>
						<!-- <tr>
					...
					<div style="margin-left:1010px">
						<input type="submit" value="查询"  style="border-radius:15px"/>
					</div>
				</form>
			</div>
		</div>
		<div class="container-fluid">
			...
			<div class="row-fluid">
				<div class="span12">
								...
								<div class="">
									<div id="DataTables_Table_0_length" class="dataTables_length">
										...
										<div class="search-box">
											<form class="searchform" method="post" action="ClassInfo_findByID.action" onsubmit="return checkClassId()">
												<input type="text" placeholder="班级编号查询..." required="" name="classinfoId" value="" id="classinfoId">
									...
									<table class="table table-bordered data-table">
										...
										<tbody>
											<s:iterator var="p" value="#session.clalist">
												<tr>
													<td style="text-align:center"><s:property
															value="#p.classid" /></td>
													<td style="text-align:center"><s:property
															value="#p.classname" /></td>
													<td style="text-align:center"><s:property
															value="#p.classnum" /></td>
													<td style="text-align:center"><s:property
															value="#p.grade" /></td>
        	        ...
        	        <s:else>
        	        	<a href="/CSManagement/ClassInfo_findByCondition.action?page=<s:property value="page-1"/>">上一页</a>
        	        </s:else>
        	        <s:iterator begin="1" end="totalPage" var="k">
        	        	<s:if test="page==#k">
        	        		<a href="/CSManagement/ClassInfo_findByCondition.action?page=<s:property value="#k"/>" class="current_page"><s:property value="#k"/></a>
        	        	</s:if>
        	        	<s:else>
        	        		<a href="/CSManagement/ClassInfo_findByCondition.action?page=<s:property value="#k"/>"><s:property value="#k"/></a>
        	        	</s:else>
        	        </s:iterator>
        	        ...
        	        <s:else>
        	        	<a href="/CSManagement/ClassInfo_findByCondition.action?page=<s:property value="page+1"/>">下一页</a>
        	        </s:else>
                </div>
					</div>
				</div>
